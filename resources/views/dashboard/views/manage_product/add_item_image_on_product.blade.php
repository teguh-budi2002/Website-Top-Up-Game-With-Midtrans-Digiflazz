  ...
  <div class="flex justify-center">
    <div class="w-2/6 bg-white shadow-md rounded p-4 mt-10">
      <p class="text-primary font-semibold uppercase">{{ $product->product_name }}</p>
        <form action="{{ URL('dashboard/add-image-item-on-product/process/' . $product->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            <div class="image_foritem_on_product w-full">
                <label for="custom_item_img_product" class="block mt-3 text-slate-600 dark:text-primary-dark text-sm">Add Item Photo Into Product</label>
                <div x-data="previewImage()">
                    <label class="cursor-pointer" for="itemImgOnProduct">
                    ...
                    <input class="w-96 cursor-pointer mt-3 focus:outline-0" type="file" name="item_img" id="itemImgOnProduct"
                        @change="fileChosen">
                </div>
            </div>
